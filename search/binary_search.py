...
def smart_binary_search(search_key, search_list):
    i = 0
    j = len(search_list) - 1
    ...
    while i < j:
        ...
        middle_element = i + (j - i) // 2
        if search_key == search_list[middle_element]:
            print("Smart search finds item in the middle, breaking now...")
            # You can just return i here, or do:
            i = middle_element
            break
...
index = smart_binary_search(search_key, search_list)
